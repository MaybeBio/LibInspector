...
class AdvancedFlowVisitor(ast.NodeVisitor):
    ...
    def visit_FunctionDef(self, node):
        ...
        if args:
            node_id = "Input"
            # Use HTML tags to enhance display (bold title)
    ...
    def _handle_assign(self, node, targets, annotation=None):
        ...
        label = "Assign"
        node_type = "process" # Default to normal processing
        ...
        if isinstance(node.value, ast.Call):
            ...
            # Highlight core transformations: AI models, mathematical operations, I/O operations
            # These are what beginners most care about inside the "Black Box"
            core_keywords = [
                ...
                'read', 'load', 'save', 'write', 'get', 'post' # I/O operations
            ]
        ...
        # C. Constant Assignment (usually configuration, beginners don't need to care unless default values)
        elif isinstance(node.value, ast.Constant):
             return # Directly ignore to reduce chart noise
        
        # --- 2. Output Variable Handling (with type annotations) ---
        outputs = []
        output_labels = []
        ...
        if outputs:
            ...
            # Use HTML line breaks to separate "Action" and "Result" layers
    ...
    # --- Added: Control Flow Visualization (Decision Logic) ---
    ...
    def visit_Return(self, node):
        ...
        if node.value:
            ...
            if hasattr(ast, 'unparse'):
                try: 
                    # Remove newlines to prevent breaking Mermaid syntax
...
def generate_function_flowchart(func_obj) -> str:
    ...
    for node in visitor.nodes:
        ...
        for source_id, var_name in node.edges_in:
            ...
            # Mermaid syntax error when adding edge labels
            # Incorrect: A --> "label" B
            # Correct: A -->|label| B
            if safe_var:
...
def convert_md_to_html(md_content: str, title: str) -> str:
    ...
    for i, part in enumerate(parts):
        if i % 2 == 0:
            ...
            # 2. Restore the specific HTML tags we generated
            ...
            if part.startswith("mermaid"):
                ...
                # Perform HTML escaping on Mermaid code
