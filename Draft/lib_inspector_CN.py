# Below is the Chinese version of the original code. For the formal version, we will use English.

# 下面是原始中文草稿(主要是一些中文注释), 正式全英文版本我们放在src中
...
class GlobalCallGraphVisitor(ast.NodeVisitor):
    ...
    def __init__(self, known_functions: Set[str]):
        ...
        self.calls = [] # 调用关系列表，格式为 (caller, callee, arg_names)
...
class EntryAnalysisVisitor(ast.NodeVisitor):
    ...
    def __init__(self):
        ...
        self.args = [] # List of (arg_name, help_text) 参数名和帮助文本的列表
    ...
    for i, part in enumerate(parts):
        if i % 2 == 0:
            ...
            if part.startswith("mermaid"):
                ...
                # 这能完美解决 Syntax error 问题 ——> 更正, 还是有问题, 问题貌似不出在Mermaid内容上, preview能渲染, 但实际页面不行
...
def inspect_library(
    ...
    try:
        ...
        if hasattr(main_module, "__path__"):
            for importer, modname, ispkg in pkgutil.walk_packages(main_module.__path__, main_module.__name__ + "."):
                # --- 1. 跳过测试模块 (关键修改) ---
                # pandas 等库包含大量测试文件，这些文件经常引用缺失的依赖(如 pyarrow)导致崩溃
                # 且测试代码通常不是架构分析的重点
                ...
                try:
                    ...
                    # --- 2. 捕获 BaseException (关键修改) ---
                    # pytest.skip() 抛出的异常继承自 BaseException，普通 Exception 抓不到
                    # 这样可以忽略因缺少可选依赖而导致的导入错误
                    continue
    finally:
        sys.argv = _old_argv
    ...
    if entry_visitor.has_main_block:
        ...
        if hasattr(main_module, "__path__"):
             # 简单扫描一下主模块和直接子模块
            scan_modules = [main_module] + submodules # 限制数量避免太慢submodules[:10], 还是全部扫描吧
        else:
        ...
        if api_candidates:
            ...
            # 这里实测确实函数比较多, 到时候可以在这里调整数量, 暂时定10个
    ...
    # ==========================================================
    # Phase 1: Network Construction & Analysis 网络分析与架构图谱
    # ==========================================================
    ...
    # =========================================================================
    # Phase 1.5: Global Call Graph & Extraction Guide 全局调用图与提取指南(新增)
    # =========================================================================
    ...
    # =========================================================
    # Phase 2: Surface Level Inspection & Logic Flow 逻辑流程
    # =========================================================
